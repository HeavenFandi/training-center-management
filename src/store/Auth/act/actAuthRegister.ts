...
>("auth/actAuthRegister", async (data, thunkAPI) => {
  ...
  try {
    ...
    const registrationResponse = await axiosClient.post(
      "/students",
      registrationPayload,
      {
        headers: {
    ...
    const studentId =
      registrationResponse.data?.id || registrationResponse.data?.data?.id;

    if (!studentId) {
      console.error(
        "[DEBUG actAuthRegister] Could not find student ID in registration response",
      );
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (status === 400) {
        ...
        if (serverData?.errors && Object.keys(serverData.errors).length > 0) {
          console.error(
            "[DEBUG actAuthRegister] 400 Bad Request - Field Errors:",
            serverData.errors,
          );
      ...
      if (status === 409) {
        ...
        console.error(
          "[DEBUG actAuthRegister] 409 Conflict - Detailed Message:",
          message,
        );
      ...
      if (status === 500) {
        console.error(
          "[DEBUG actAuthRegister] 500 Internal Server Error - Check backend logs",
        );
        const errorMsg =
          typeof serverData === "string"
            ? serverData
