...
const SchedulingConflictDialog: React.FC<Props> = ({
  ...
  const conflictCount =
    conflictData.conflictingDates?.length ||
    conflictData.conflicts?.length ||
    conflictData.count ||
    (conflictData.message?.match(/\d+/)
      ? parseInt(conflictData.message.match(/\d+/)![0])
      : 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {conflictData.availableHalls &&
          conflictData.availableHalls.length > 0 && (
            <Box sx={{ mb: 3 }}>
              <Typography
                variant="subtitle1"
                fontWeight="bold"
              ...
              >
                <MeetingRoomIcon
                  sx={{ fontSize: 20, verticalAlign: "middle", ml: 0.5 }}
                />
                القاعات المتاحة
              </Typography>
                    ...
                    label={
                      <Box sx={{ textAlign: "center" }}>
                        <Box sx={{ fontWeight: "bold" }}>
                          {hall.name || hall.number || `قاعة ${index + 1}`}
                        </Box>
                        {hall.capacity && (
